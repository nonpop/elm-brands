module Brands.Foldable exposing (All(..), Any(..), Foldable, Max(..), Min(..), Sum(..), foldMap, isEmpty, length, maximum, member, minimum, monoidAll, monoidAny, monoidList, monoidMax, monoidMin, monoidSum, runAll, runAny, runMax, runMin, runSum, toList)
...
length foldable =
    runSum << foldable.foldMap monoidSum (\_ -> Sum 1)


toList : Foldable f a (List a) -> App f a -> List a
toList foldable =
    foldable.foldMap monoidList (\x -> [ x ])


isEmpty : Foldable f a All -> App f a -> Bool
isEmpty foldable =
    runAll << foldable.foldMap monoidAll (\_ -> All False)


member : Foldable f a Any -> a -> App f a -> Bool
member foldable x =
    runAny
        << foldable.foldMap monoidAny
            (\y ->
                if x == y then
                    Any True
...
maximum foldable =
    runMax << foldable.foldMap monoidMax (Max << Just)


minimum : Foldable f comparable (Min comparable) -> App f comparable -> Maybe comparable
minimum foldable =
    runMin << foldable.foldMap monoidMin (Min << Just)
